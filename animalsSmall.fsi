...
type environment =
  class
    ...
    /// <param name="time">the length of the simulation</param>
    /// <param name="fileName">the file name of the results file</param>
    ...
    new : time : int * fileName : string * boardWidth:int * NMooses:int * mooseRepLen:int * NWolves:int *
    ...
    member size : int
    /// Perform a tick by performing all animal's ticks in random order. 
    /// Animals perform the following actions: Calves and cubs are added if there is room in a neighbouring position.
